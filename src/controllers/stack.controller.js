...
class StackController {
    static async createStack(req, res, next) {
        try {
            ...
            if (style) {
                ...
                const ESTILOS_PERMITIDOS = ['Casual', 'Formal', 'Deportivo', 'Semiformal'];
    ...
    static async getTodaysOutfit(req, res) {
        try {
            const outfit = await StackService.getCurrentDayOutfit(req.user.id_usuario);
            res.status(200).json({
                success: true,
                data: outfit
            });
