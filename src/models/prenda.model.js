...
const PrendaModel = {
  async registrarPrenda(dto) {
    const query =
      "SELECT * FROM sp_registrar_prenda($1, $2, $3, $4, $5, $6, $7, $8)";
    const values = [
      dto.id_usuario,
      dto.nombre_prenda,
    ...
    try {
      ...
      if (
        result.rows[0].sp_registrar_prenda !== "Prenda registrada con éxito."
      ) {
  ...
  async findByUser(userId) {
    const query = `SELECT * FROM prenda WHERE id_usuario = $1 AND status = true`;
    const { rows } = await pool.query(query, [userId]);
    return rows;
  },
